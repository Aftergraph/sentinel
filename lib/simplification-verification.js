...
export async function verifySimplificationEvidence({
  ...
  const executionStatus=text(envelope.evidence.body.executionStatus);
  if(executionStatus&&executionStatus!=='completed'){
    checks.push(check('EXECUTION_STATUS','INDETERMINATE','execution_not_completed',{executionStatus}));
    return emit(INDETERMINATE,'execution_not_completed',checks);
  }
  if(executionStatus)checks.push(check('EXECUTION_STATUS','PASS',null,{executionStatus}));
