...
async function readJson(fetchImpl,url,token,timeoutMs){
  ...
  if(!response?.ok)return {ok:false,kind:response?.status===404?'missing':'unavailable'};
...
export function createGitHubSimplificationObserver({
  ...
  if(typeof token!=='string'||Buffer.byteLength(token,'utf8')<20)throw new Error('github_token_required');
