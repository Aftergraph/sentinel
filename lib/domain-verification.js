...
function makeReceipt({envelope,verdict,checks,verifierRef,verifiedAt}){
  const subjectId=subjectIdFor(envelope);
  const verificationId=`dv_${hashBody({subjectId,verifierRef,verifiedAt}).slice(0,32)}`;
  const body={
  ...
  const receiptDigestSha256=hashBody(body);
...
function result({envelope,verdict,reason,checks,verifierRef,verifiedAt}){
  const receipt=makeReceipt({envelope,verdict,checks,verifierRef,verifiedAt});
  return Object.freeze({
...
export async function verifyDomainEvidence({envelope,independentCheck,verifierRef,now=()=>new Date().toISOString()}={}){
  ...
  return result({
    envelope,verdict,reason,checks,verifierRef:verifier,
    verifiedAt:now(),
  });
}
