...
import { verifySimplificationVerificationReceipt } from './simplification-verification.js';
...
function receiptFamily(receipt){
  if(verifyDomainVerificationReceipt(receipt))return 'domain';
  if(verifySimplificationVerificationReceipt(receipt))return 'simplification';
  return null;
}
function assertReceipt(receipt){
  if(!receipt||typeof receipt!=='object'||Array.isArray(receipt))throw new Error('domain-verification-store: invalid receipt');
  if(!receiptFamily(receipt))throw new Error('domain-verification-store: reject tampered receipt digest');
  return true;
  ...
  const save=()=>persist(filePath,state);  const store={
    ...
    put(receipt){
      ...
      append('domain-verification.stored',{
        findingId:null,
        from:stored.subjectId??stored.missionId??null,
        to:stored.receiptId,
    ...
    listBySubject(subjectId){
      ...
      for(const item of state.receipts){
        try{if(!receiptFamily(item))bad.push(item?.receiptId??String(item?.receiptId));}
