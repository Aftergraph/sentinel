...
import { blastRadius, buildRepoGraph } from './context-graph.js';
// Additive: verdict-override audit trail (append-only, no other coupling).
import { append as appendAuditEvent } from './audit.js';

// Fail-closed argv builder for `gh api` (never interpolate user input
// into a shell string: owner/name, endpoint charset and length are
// validated, then passed as argv with shell:false).
export function ghApiArgv(repo, endpoint) {
  if (typeof repo !== 'string' || !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repo)) {
    throw new Error(`Invalid repo (expected owner/name): ${String(repo).slice(0, 80)}`);
  }
  if (typeof endpoint !== 'string' || endpoint.length === 0 || endpoint.length > 200
    || !/^[A-Za-z0-9/_.-]+$/.test(endpoint) || endpoint.split('/').includes('..')) {
    throw new Error(`Invalid gh api endpoint: ${String(endpoint).slice(0, 80)}`);
  }
  return ['api', `repos/${repo}/${endpoint}`];
}
...
// Additive attestation passthrough: policyEvaluation + overridden flow into
// gov/sarif outputs without touching any pre-existing field shape.
function attestationOf(result) {
  return {
    ...(result.policyEvaluation != null ? { policyEvaluation: result.policyEvaluation } : {}),
    ...(result.overridden != null ? { overridden: result.overridden, overriddenFrom: result.overriddenFrom } : {}),
  };
}
  ...
  return {
    ...
    sarif: {
      ...
      runs: [{
        ...
        ...(Object.keys(attestation).length > 0 ? { properties: attestation } : {}),
      }]
    }
  };
}

// Advisory blast-radius context (S2 slice 2) — "what does this finding
// touch?" computed from lib/context-graph.js. Deterministic, capped, and
// explicitly NOT a verdict signal: it never enters receipt hashes (see
// stripBlastContext in lib/receipt.js) or verdict logic. Findings are
// never mutated — withBlastContext returns copies.
export const BLAST_FILES_CAP = 5;
export const BLAST_SYMBOLS_CAP = 5;
export const BLAST_TESTS_CAP = 5;
export const BLAST_CONTEXT_NOTE = 'advisory blast-radius context only — never affects the verdict or the receipt';

const TEST_PATH_RE = /(^|\/)(__tests__|test|tests|spec|specs)\//i;
const TEST_BASENAME_RE = /(\.test\.|\.spec\.|_test\.|^test_)/i;

export function isTestPath(path) {
  const p = String(path || '');
  const base = p.split('/').pop();
  return TEST_PATH_RE.test(p) || TEST_BASENAME_RE.test(base);
}

function capCopy(list, cap) {
  const items = Array.isArray(list) ? list.slice() : [];
  if (items.length <= cap) return { items, truncated: false };
  return { items: items.slice(0, cap), truncated: true };
}

// Pure: advisory context for one finding over a prebuilt graph (see
// buildGraph/buildRepoGraph in lib/context-graph.js). The symbol scopes
// via enclosingSymbol from the finding line. Returns null only when there
// is no graph or no finding file — an unknown file yields empty lists
// (still advisory, still a uniform shape).
export function findingBlastContext(finding, graph, { filesCap = BLAST_FILES_CAP, symbolsCap = BLAST_SYMBOLS_CAP, testsCap = BLAST_TESTS_CAP } = {}) {
  if (!finding || typeof finding.file !== 'string' || !graph || !graph.files) return null;
  const blast = blastRadius(graph, { file: finding.file, line: typeof finding.line === 'number' ? finding.line : undefined });
  const files = capCopy(blast.files, filesCap);
  const names = [];
  for (const s of blast.symbols || []) {
    if (s && typeof s.name === 'string' && !names.includes(s.name)) names.push(s.name);
  }
  const symbols = capCopy(names, symbolsCap);
  const tests = capCopy(blast.files.filter(isTestPath), testsCap);
  return {
    files: files.items,
    symbols: symbols.items,
    tests: tests.items,
    truncated: files.truncated || symbols.truncated || tests.truncated,
  };
}

// Pure: copy each finding with a `blastContext` field attached (null when
// no context applies). The input array and its findings are never mutated.
export function withBlastContext(findings, graph, opts = {}) {
  return (findings || []).map((f) => ({ ...f, blastContext: findingBlastContext(f, graph, opts) }));
}

// Copy a finding list for JSON output, attaching fresh `blastContext` per
// finding only when a graph is supplied; pre-attached contexts pass
// through on copies; otherwise the list is returned as-is so default
// output stays byte-identical.
function blastList(list, graph) {
  if (graph != null) return (list || []).map((f) => ({ ...f, blastContext: findingBlastContext(f, graph) }));
  if ((list || []).some((f) => f && f.blastContext !== undefined)) return (list || []).map((f) => ({ ...f }));
  return list;
}

// Render one finding's advisory context for human output. Prefers a
// pre-attached `blastContext`, else computes from `graph`. Null when
// neither is available or the context is empty.
function blastLineFor(finding, graph) {
  const ctx = (finding && finding.blastContext !== undefined && finding.blastContext !== null)
    ? finding.blastContext
    : (graph ? findingBlastContext(finding, graph) : null);
  if (!ctx) return null;
  const parts = [];
  if (ctx.files && ctx.files.length > 0) parts.push(`files(${ctx.files.length}): ${ctx.files.join(', ')}`);
  if (ctx.symbols && ctx.symbols.length > 0) parts.push(`symbols: ${ctx.symbols.join(', ')}`);
  if (ctx.tests && ctx.tests.length > 0) parts.push(`tests: ${ctx.tests.join(', ')}`);
  if (parts.length === 0) return null;
  return `    ~ context (advisory, non-verdict): ${parts.join('; ')}${ctx.truncated ? ' (truncated)' : ''}`;
}

function blastLegendFor(result, graph) {
  if (graph != null) return true;
  for (const list of [result.blocking, result.silenced, result.nonBlocking, result.excluded]) {
    if ((list || []).some((f) => f && f.blastContext != null)) return true;
  }
  return false;
}
...
export function formatHuman(result, { summary = null, delta = null, receipt = null, staleReason = null, color = false, blastGraph = null } = {}) {
  const lines = [];
  if (summary) {
  ...
  if (staleReason || result.verdict === 'STALE') {
    ...
    for (const f of result.blocking) {
      ...
      const blast = blastLineFor(f, blastGraph);
  ...
  if (blastLegendFor(result, blastGraph)) lines.push(`context: ${BLAST_CONTEXT_NOTE}`);
...
export function toJson(result, { repo = null, prNumber = null, summary = null, delta = null, receipt = null, blastGraph = null } = {}) {
  ...
  return {
    ...
    findings: {
      blocking: blastList(result.blocking, blastGraph),
      silenced: blastList(result.silenced, blastGraph),
      nonBlocking: blastList(result.nonBlocking || [], blastGraph),
      excluded: blastList(result.excluded || [], blastGraph),
    },
    summary,
    delta: delta ? {
    ...
    receipt,
    // Additive: present only when blast context was attached (advisory, never hashed).
    ...(blastLegendFor(result, blastGraph) ? { contextNote: BLAST_CONTEXT_NOTE } : {}),
...
export async function analyzeDiff({ diffText, pack, excludePatterns = [], resolutions = new Set(), headSha, baseSha, override }) {
  ...
  // Exact duplicates (same rule + file + line + evidence, e.g. two sliding
  // windows covering one line) carry no independent signal: keep the first.
  // Counts stay honest — one emission per distinct observation.
...
export async function review({
  ...
  if (localMode) {
    ...
    if (!Number.isInteger(pr) || pr < 1) {
      throw new Error(`Invalid pr (expected positive integer): ${String(pr).slice(0, 20)}`);
    }
  ...
  // for human/json context lines only. Failures warn to stderr and continue
  // WITHOUT context — verdict and receipt paths never see this data.
  let blastGraph = null;
  ...
  const buildReceipt = (verdict, snap, extra = {}) => makeReceipt({
    repo,
    prNumber: pr,
    headSha,
    ...
    overriddenFrom: extra.overriddenFrom,
    policyEvaluation: extra.policyEvaluation,
    counts: {
  ...
  const receipt = buildReceipt(result.verdict, snapshot, result);
  if (!noLedger) appendLedger(receipt, ledgerFile);
  ...
  function emit(res, extra) {
    if (format === 'sarif') {
      ...
      console.log(JSON.stringify(toJson(res, { repo, prNumber: pr, blastGraph, ...extra }), null, 2));
    } else if (format === 'gov') {
