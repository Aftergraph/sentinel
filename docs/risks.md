...
7. **Receipt chain is tamper-evident, not tamper-proof (known limitation).** `receipt_id` commits to content but not to `prev_receipt_id`, so a writer with ledger-file access can rewrite a suffix self-consistently; single-line drops ARE caught (linkage break → `/api/ledger/verify` reports `ok:false`). Trust boundary is the OS/user owning the ledger file. Mitigation: restrictive file permissions, remote/append-only ledger for hosted use; roadmap: hash-chained receipt v0.2 committing the previous id into `receipt_id` (contract change — old ids must keep verifying).
