...
| `lib/rulepack.js` | Single source of truth: rule IDs, severities, pack versions, blocking set | `RULE_PACK_VERSION` (`1.5.0`), `SUPPORTED_PACKS` (`1.0.0\|1.1.0\|1.2.0\|1.3.0\|1.4.0\|1.5.0`), `RULE_IDS_BY_PACK`, `SEVERITY_MAP`, `BLOCKING_SEVERITIES`, `ruleIdsForPack` |
...
1.4.0 = 23, 1.5.0 = 24 (pinned by `test/cli-contract.test.mjs`).
...
| `require-health-check-before-traffic-shift` | reliability | Container image with no readiness/liveness probe (v1.4.0 addition) |
| `require-foreign-key-constraints-on-related-tables` | data | `*_id` column with no FK constraint (v1.5.0 addition) |
...
`--rule-pack 1.0.0|1.1.0|1.2.0|1.3.0|1.4.0|1.5.0`, `--policy`, `--override
