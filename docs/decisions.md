...
| 13 | Pack v1.5.0 promotes gap #14 (FK rule, first schema-aware rule); gaps #12/#19 stay out | `*_id` column without REFERENCES in the same file's added lines is a binary file-local signal (polymorphic `*_type` and bare `id` excluded); #12/#19 need deploy-model and cross-statement index context | Yes (precision is the moat) |
| 14 | Pack v1.6.0 promotes gap #9 (replicas:1+Recreate combo rule); gaps #12/#19 stay out | The combination is a binary downtime signal where each alone is legitimate (singletons, queues, dev); #12/#19 still need deploy-model and cross-statement index context no same-file formulation delivers | Yes (precision is the moat) |
| 15 | Pack v1.7.0 adds no-hardcoded-api-token-in-diff (provider grammars only); generic secret scanners stay out, `sk-test-*` stays out | Provider prefix + fixed shape is binary (AWS/GH/Stripe/Slack/Google); entropy scanning and test-mode keys have no binary legitimate-vs-secret split at diff scope; official example values hit the hatch | Yes (precision is the moat) |
